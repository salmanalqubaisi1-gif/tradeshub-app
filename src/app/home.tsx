...
type TabName = 'Feed' | 'Jobs' | 'Career' | 'Profile';

type HoursView = 'week' | 'month' | 'year' | 'all';
...
const ROLE_TABS: Record<TradesHubRole, RoleTab[]> = {
  tradesperson: [
...
type TrainingProviderSuggestion = {
  provider_id: string;
...
function getProviderSearchAliases(providerName: string): string[] {
...
function normalizeTrainingProviderSearchText(value: string): string {
  return value
    .toLowerCase()
    .replace(/&/g, ' and ')
    .replace(/[^a-z0-9]+/g, ' ')
...
// V1 client-side moderation for public-facing user content.
// Server-side enforcement should mirror this before public launch.
...
function moderationMessage(): string {
  return 'Please remove profanity, slurs, threats or abusive language.';
}

function getTradeWorkTypes(trade: string): string[] {
  ...
  if (
    value.includes('pipefitter') ||
    value.includes('steamfitter')
  ) {
    return [
      'Industrial Piping',
      'Commercial Piping',
  ...
  if (
    value.includes('millwright') ||
    value.includes('industrial mechanic')
  ) {
    return [
      'Installation',
  ...
  if (
    value.includes('heavy duty') ||
    value.includes('heavy equipment technician')
  ) {
    return [
      'Diagnostics',
      'Preventive Maintenance',
...
const JOURNEYPERSON_EXPERIENCE_OPTIONS = [
  'New Journeyperson',
  '1+ years experience',
  '3+ years experience',
  '5+ years experience',
